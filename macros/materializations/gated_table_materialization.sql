...
-- If it's a failures model, we want to inject the filters into the SELECT statement to see how it fails.
...
{%- materialization gated_table, adapter='bigquery' -%}
    ...
    -- Create model as a normal table materialization
    {% call statement('main') -%}
  ...
  {% do persist_docs(gate_failures_relation, model) %}
